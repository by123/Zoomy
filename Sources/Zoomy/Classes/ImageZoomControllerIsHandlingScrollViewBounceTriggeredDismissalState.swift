...
class
  ImageZoomControllerIsHandlingScrollViewBounceTriggeredDismissalState:
    ImageZoomControllerIsPresentingImageViewOverlayState
{
  ...
  override func didPan(with gestureRecognizer: UIPanGestureRecognizer) {
    ...
    if gestureRecognizer.state == .changed {
      ...
      owner.backgroundView.alpha = backgroundAlpha(
        for: translation.value(in: translation.dominantDirection))
    } else if gestureRecognizer.state != .began {  //.ended, .failed, or .cancelled
      guard !isDismissingOverlay else { return }

      if translation.value(in: translationDirection)
        >= owner.settings.neededTranslationToDismissOverlayOnScrollBounce {
...
extension ImageZoomControllerIsHandlingScrollViewBounceTriggeredDismissalState {

  fileprivate func scale(for translation: CGPoint) -> ImageViewScale {
    ...
    return 1
      - (min(
        translation.value(in: translationDirection), translationForMinimumZoom)
        / translationForMinimumZoom) * delta
  }

  fileprivate func backgroundAlpha(for translation: CGFloat) -> CGFloat {
    guard let owner = owner else { return 0 }
    let progress =
      (translation / owner.settings.backgroundAlphaDismissalTranslationThreshold)
