...
extension ImageZoomController.Factory {
  ...
  func makeTapGestureRecognizer(for context: Context) -> UITapGestureRecognizer{
    let gestureRecognizer = UITapGestureRecognizer(
      target: context, action: #selector(context.didTap(with:)))
    gestureRecognizer.delegate = context
    return gestureRecognizer
  }

  func makeDoubleTapGestureRecognizer(for context: Context)
    -> UITapGestureRecognizer {
    let gestureRecognizer = UITapGestureRecognizer(target: context, action: #selector(context.didDoubleTap(with:)))
    gestureRecognizer.numberOfTapsRequired = 2
    gestureRecognizer.delegate = context
    return gestureRecognizer
  }

  func makePinchGestureRecognizer(for context: Context)
    -> UIPinchGestureRecognizer {
    let gestureRecognizer = UIPinchGestureRecognizer(
      target: context, action: #selector(context.didPinch(with:)))
    gestureRecognizer.delegate = context
    return gestureRecognizer
  }
  ...
  func makePanGestureRecognizer(for context: Context) -> UIPanGestureRecognizer {
    let gestureRecognizer = UIPanGestureRecognizer(
      target: context, action: #selector(context.didPan(with:)))
    gestureRecognizer.delegate = context
    return gestureRecognizer
  }
}
...
extension ImageZoomController.Factory {
  ...
  func makeScrollableImageView(for context: Context) -> UIImageView {
    ...
    view.addGestureRecognizer(context.scrollableDoubleTapGestureRecognizer)
