...
class ImageZoomControllerIsPresentingScrollViewOverlayState: NSObject {
  ...
  private let maximumPanTranslationDuringBounceThatTriggersStateChange:
    CGFloat = 20

  private var contentState = ContentState.smallerThanAnsestorView {
    didSet {
      guard let owner = owner,
        contentState != oldValue
      else { return }

      print("Changed from \(oldValue) to \(contentState)")

      owner.animator(for: .backgroundColorChange).animate {
        owner.backgroundView.backgroundColor = self.backgroundColor(
          for: self.contentState)
...
extension ImageZoomControllerIsPresentingScrollViewOverlayState:
  ImageZoomControllerState
{
  ...
  func presentOverlay() {
    guard let owner = owner,
      ...
      let dominantBouncingSide = dominantBouncingDirection
    else { return }

    if let topmostView = owner.topmostView {
      containerView.insertSubview(
        owner.overlayImageView, belowSubview: topmostView)
    } else {
    ...
    owner.state = IsHandlingScrollViewBounceTriggeredDismissalState(
      owner: owner, translationDirection: dominantBouncingSide)
  ...
  func zoomIn(with gestureRecognizer: UIGestureRecognizer?) {
    ...
    if let location = gestureRecognizer?.location(in: gestureRecognizer?.view) {
      ...
      scrollView.zoom(
        to: CGRect(x: location.x, y: location.y, width: 0, height: 0),
        animated: true)
    } else {
      print("zoomScale: \(scrollView.maximumZoomScale)")
  ...
  func didPan(with gestureRecognizer: UIPanGestureRecognizer) {
    ...
    guard isZooming == false,
      ...
      let dominantBouncingDirection = currentBounceOffsets.bouncingSides()
        .first,
      translation.dominantDirection == dominantBouncingDirection,
      translation.value(in: dominantBouncingDirection)
        < maximumPanTranslationDuringBounceThatTriggersStateChange
    else { return }
    ...
    print(" Will bounce. translation: \(translation) ")
  ...
  func scrollViewDidEndZooming(
    ...
    if scrollView.zoomScale <= owner.minimumZoomScale
      || scrollView.zoomScale
        <= owner.zoomScale(from: owner.settings.zoomCancelingThreshold) {
      dismissOverlay()
    }
  }
...
extension ImageZoomControllerIsPresentingScrollViewOverlayState {
  ...
  fileprivate func didStartBouncingScroll(in direction: Side) {
    perform(action: action(for: direction))
  }

  fileprivate func action(for side: Side) -> Action {
  ...
  fileprivate func neededOverlayImageViewFrame() -> CGRect {
    guard let owner = owner,
      let image = owner.image
    else { return CGRect.zero }

    return CGRect(
      origin: owner.absoluteFrame(of: owner.scrollableImageView).origin,
      size: owner.size(of: image, at: owner.scrollView.zoomScale))
  }

  fileprivate func backgroundColor(for state: ImageZoomControllerContentState)
    -> UIColor {
  ...
  fileprivate func neededContentState() -> ImageZoomControllerContentState {
    guard let containerView = owner?.containerView,
      let scrollView = owner?.scrollView
    else { return .smallerThanAnsestorView }
    ...
    return scrollView.contentSize.width >= containerView.frame.size.width
      ...
      ? .fillsAnsestorView : .smallerThanAnsestorView
  }
}
